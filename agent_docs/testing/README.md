...
- Workflow edit-intent smoke:
  - `python3 scripts/ollama/smoke.py --edit-intent-only`
- Workflow failure fixture check:
...
- `--edit-intent-only` returns JSON with `"status": "edit-intent-passed"` plus markdown target path, `replace_content` operation, and replacement-content summary fields.
- `--workflow-failure-fixture-only` returns JSON with `"status": "workflow-failure-fixture-passed"` plus planner and draft failure summaries.
...
- Draft responses that are non-JSON, omit the `edit_intent` object, or return invalid field types.
- Edit-intent outputs that target non-markdown paths, include traversal-like paths, use unsupported operations, or return a path that does not match the requested note.
...
- Workflow failure fixture expectations should stay bounded to `category`, `boundary`, and `owner` for local planner/draft failures; do not expand them into `mirai` endpoint or MCP error-envelope assertions.
